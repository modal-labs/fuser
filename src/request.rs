        ...
        match op {
            ...
            ll::Operation::Init(x) => {
                ...
                if let Err(e) = se.filesystem.init(self, &mut config) {
                    return Err((Errno::from_i32(e), sender));
                }
            ...
            ll::Operation::Poll(x) => {
                // The poll handle outlives this dispatch, so the sender is
                // type-erased into an `Arc` that both it and the reply can hold.
                let sender: Arc<dyn ReplySender> = Arc::new(sender);
            ...
            ll::Operation::GetXTimes(x) => {
                se.filesystem
                    .getxtimes(self, x.nodeid().into(), self.reply(sender));
            }
            #[cfg(target_os = "macos")]
